...
const getCartItemProductId = (item) => item.product?._id || item.product;
...
router.post('/add', authenticate, async (req, res, next) => {
  try {
    ...
    const selectedColor = normalizeColorName(req.body.color || req.body.selectedColor || 'Mặc định');
    ...
    const index = cart.items.findIndex((item) =>
      String(getCartItemProductId(item)) === String(product._id)
...
router.put('/update', authenticate, async (req, res, next) => {
  try {
    ...
    const item = cart.items.find((cartItem) =>
      String(getCartItemProductId(cartItem)) === String(productId)
...
router.delete('/remove/:productId/:size/:color', authenticate, async (req, res, next) => {
  try {
    ...
    cart.items = cart.items.filter((item) => !(
      String(getCartItemProductId(item)) === String(req.params.productId)
