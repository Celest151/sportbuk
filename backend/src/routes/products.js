...
const normalizeLegacyVariants = (req, existingProduct = null) => {
  ...
  const colorName = normalizeLegacyColors(req.body.color || existingVariant.color || existingProduct?.color || 'Mặc định')[0] || 'Mặc định';
...
const buildProductPayload = async (req, existingProduct = null) => {
  ...
  const rawVariants = parseJsonField(req.body.variants, null, 'variants');
  const variants = normalizeVariantsInput(rawVariants);
  if (rawVariants !== null && (!Array.isArray(rawVariants) || rawVariants.length === 0 || variants.length !== rawVariants.length)) {
    throw new AppError('Each product variant requires a valid color, size, price, and stock value', 400);
  }
  ...
  if (!primaryImage) {
    throw new AppError('Products require at least one image', 400);
  }
  ...
  return {
    ...
    colorOptions,
    sizeGuideType: ['tops', 'bottoms', 'none'].includes(req.body.sizeGuideType)
      ? req.body.sizeGuideType
      : existingProduct?.sizeGuideType || 'none',
