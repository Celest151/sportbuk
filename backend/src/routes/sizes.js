...
const normalizePayload = (body = {}, existingSize = null) => {
  const name = String(body.name || '').trim();
  const defaultMeta = SIZE_META_MAP.get(name) || {};
  const textValue = (field) => String(body[field] ?? existingSize?.[field] ?? '').trim();

  return {
    name,
    code: defaultMeta.code || name.replace(/\s+/g, '').toUpperCase(),
    displayOrder: Number(defaultMeta.displayOrder || 999),
    usSize: textValue('usSize'),
    euSize: textValue('euSize'),
    height: textValue('height'),
    weight: textValue('weight'),
    chest: textValue('chest'),
    waist: textValue('waist'),
    hip: textValue('hip'),
    fitNote: textValue('fitNote'),
    isActive: body.isActive === undefined ? existingSize?.isActive !== false : body.isActive === true || body.isActive === 'true'
  };
};
...
router.put('/:id', authenticate, adminOnly, async (req, res, next) => {
  try {
    ...
    const payload = normalizePayload(req.body, existingSize);
    if (!payload.name) {
      return res.status(400).json({ success: false, message: 'Tên size là bắt buộc' });
    }
