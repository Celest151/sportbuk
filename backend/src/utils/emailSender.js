...
class EmailSender {
  ...
  async sendOrderConfirmation(userEmail, orderData) {
    const htmlContent = `
      ...
      <p>Best regards,<br>SPORTBUK Team</p>
    `;
    ...
    try {
      await this.transporter.sendMail({
        ...
        to: userEmail,
        subject: `SPORTBUK - Order Confirmation - ${orderData.orderCode}`,
        html: htmlContent
      });
  ...
  async sendPasswordReset(userEmail, resetLink, userName = '') {
      ...
      <div style="font-family: Arial, Helvetica, sans-serif; background:#f8f4ef; padding:24px; color:#2b2b2b;">
        <div style="max-width:640px; margin:0 auto; background:#ffffff; border-radius:16px; overflow:hidden; border:1px solid #eee3da;">
          <div style="padding:28px 32px; background:linear-gradient(135deg, #fff1e4 0%, #fffaf5 100%); border-bottom:1px solid #f0e3d7;">
            <div style="font-size:13px; letter-spacing:0.12em; text-transform:uppercase; color:#b85c38; font-weight:700; margin-bottom:8px;">SPORTBUK</div>
    ...
    try {
      await this.transporter.sendMail({
        from: process.env.EMAIL_FROM,
        to: userEmail,
        subject: 'SPORTBUK - Password Reset Request',
        html: htmlContent
      });
      return { success: true };
    } catch (error) {
  ...
  async sendPaymentNotification(userEmail, paymentData) {
    ...
    try {
      await this.transporter.sendMail({
        from: process.env.EMAIL_FROM,
        to: userEmail,
        subject: 'SPORTBUK - Payment Confirmation',
        html: htmlContent
      });
      return { success: true };
    } catch (error) {
  ...
  async sendInvoiceEmail(userEmail, invoiceData) {
    ...
    try {
      const info = await this.transporter.sendMail({
        from: process.env.EMAIL_FROM,
        to: userEmail,
        subject: `SPORTBUK - Hóa đơn thanh toán - ${invoiceData.orderCode}`,
        html: htmlContent
      });
