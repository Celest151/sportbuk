...
function getStoredUser() {
    const rawUser = localStorage.getItem(getStorageKey('USER_INFO', 'sportbuk_user_info')) || localStorage.getItem('userInfo');
    if (!rawUser) {
        return null;
    }
...
function storeUserInfo(user) {
    ...
    localStorage.setItem(getStorageKey('USER_INFO', 'sportbuk_user_info'), JSON.stringify(user));
    localStorage.setItem('userInfo', JSON.stringify(user));
}

const POST_AUTH_REDIRECT_KEY = 'sportbuk_post_auth_redirect';
...
function getUserCartStorageKey(user = getStoredUser()) {
    const baseKey = getStorageKey('CART', 'sportbuk_cart');
...
function getStoredWishlist() {
    const rawWishlist = localStorage.getItem(getStorageKey('WISHLIST', 'sportbuk_wishlist')) || localStorage.getItem('wishlist');
    if (!rawWishlist) {
        return [];
    }
...
function persistWishlist(items) {
    localStorage.setItem(getStorageKey('WISHLIST', 'sportbuk_wishlist'), JSON.stringify(items));
        ...
        authContainer.innerHTML = `
            <div class="header-account">
                ...
                <div class="header-account-menu">
                    ...
                    <a href="wishlist.html" class="header-account-link">Sản phẩm yêu thích</a>
                </div>
            </div>
            <button id="logoutBtn" class="header-logout-btn header-logout-inline">Đăng xuất</button>
        `;
...
function ensureChatWidgetAssets() {
    if (!shouldLoadChatWidget() || document.querySelector('script[data-sportbuk-chat-widget]')) {
        return;
    }

    const styleHref = getSharedAssetUrl('../css/common/chat-widget.css');
    if (styleHref && !document.querySelector('link[data-sportbuk-chat-widget]')) {
        const link = document.createElement('link');
        link.rel = 'stylesheet';
        link.href = styleHref;
        link.setAttribute('data-sportbuk-chat-widget', 'true');
    ...
    if (scriptSrc) {
        ...
        script.defer = true;
        script.setAttribute('data-sportbuk-chat-widget', 'true');
...
function clearAuthStorage() {
    ...
    localStorage.removeItem(getStorageKey('AUTH_TOKEN', 'sportbuk_auth_token'));
    localStorage.removeItem(getStorageKey('REFRESH_TOKEN', 'sportbuk_refresh_token'));
    localStorage.removeItem(getStorageKey('USER_INFO', 'sportbuk_user_info'));
    localStorage.removeItem('authToken');
    localStorage.removeItem('refreshToken');
    localStorage.removeItem('userInfo');
    localStorage.removeItem('user');
    localStorage.removeItem('token');
    localStorage.removeItem(getStorageKey('WISHLIST', 'sportbuk_wishlist'));
